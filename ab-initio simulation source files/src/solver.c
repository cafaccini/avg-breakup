...
void stokes_solver_direct(MYKSP *solver, 
  ...
  KSPSetReusePreconditioner(solver->ksp, PETSC_FALSE);
  PCSetType(pc, PCLU);
...
void stokes_solver_precond(MYKSP *solver,
  ...
  KSPSetOperators(solver->ksp,
                  B->data, B->data);
  KSPSetReusePreconditioner(solver->ksp, PETSC_TRUE);
  
  KSPSetUp(solver->ksp);
  // KSPSetType(solver->ksp, KSPGMRES);
  // KSPSetTolerances(solver->ksp, 1.e-4, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT);
  ...
  int rank;
  MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
  if (rank==0) printf("iterative solver, iter=%d\n", solver->niter);
